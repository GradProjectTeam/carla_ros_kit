...
class SensorManager:
    def __init__(self, vehicle, world):
        ...
        # self.lidar_queue = Queue(maxsize=1)  # Comment out
        self.radar_queue = Queue(maxsize=1)
        
        # TCP setup with different ports
        self.host_ip = '127.0.0.1'
        # self.lidar_port = 12349  # Comment out
        self.radar_port = 12347
        
        # Thread control
        self.running = True
        # self.lidar_thread = None  # Comment out
    ...
    def setup_tcp_sockets(self):
        ...
        #     print("LiDAR TCP client connected to {0}:{1}".format(self.host_ip, self.lidar_port))
        # except ConnectionRefusedError:
        #     print("Could not connect to LiDAR server. Is it running?")
        #     raise
        ...
        try:
            ...
            print("Radar TCP client connected to {0}:{1}".format(self.host_ip, self.radar_port))
        except ConnectionRefusedError:
            print("Could not connect to Radar server. Is it running?")
            raise
        
    def start_processing_threads(self):
        # self.lidar_thread = threading.Thread(target=self.process_lidar_queue)  # Comment out
        self.radar_thread = threading.Thread(target=self.process_radar_queue)
        
        # self.lidar_thread.daemon = True  # Comment out
        self.radar_thread.daemon = True
        
        # self.lidar_thread.start()  # Comment out
    ...
    def cleanup(self):
        ...
        # if self.lidar_thread and self.lidar_thread.is_alive():  # Comment out
        ...
        # Close sockets
        # if hasattr(self, 'lidar_socket'):  # Comment out
        #     try:
    ...
    def setup_radar(self):
        try:
            ...
            radar_transform = carla.Transform(
                carla.Location(x=4.5, z=2.0),  # x: forward, z: up
                carla.Rotation()  # Default rotation (0,0,0) will inherit car's rotation
            )
