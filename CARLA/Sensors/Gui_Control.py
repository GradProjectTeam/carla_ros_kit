...
class SensorManager:  # Class managing all vehicle sensors and their data processing
                ...
                if not self.lidar_queue.empty():
                    ...
                    for point in point_cloud:
                        ...
                        try:
                            ...
                            print(f"Error processing LiDAR point: {e}")  # Log point processing errors
    ...
    def setup_lidar(self):  # Configure and spawn LiDAR sensor
        try:
            ...
            lidar_bp.set_attribute('lower_fov', '-10.0')  # Downward scan angle
    ...
    def get_current_lane_type(self):
        """Get the current lane type of the vehicle"""
        ...
        try:
            ...
            # Get the lane type and convert to string
            lane_type = waypoint.lane_type
            
            # Debug print to see the actual lane type value
            print(f"Current lane type: {lane_type} ({lane_type_mapping.get(lane_type, 'Unknown')})")
    ...
    def has_right_lane(self):
        ...
        try:
            ...
            if right_lane is None:
                print(f"Has right lane: False (no lane)")
                return False
            ...
            if (right_lane.lane_type == carla.LaneType.Sidewalk or 
                ...
                right_lane.lane_type == carla.LaneType.Parking):
                print(f"Has right lane: False (lane type: {right_lane.lane_type})")
                return False
                
            # There is a valid right lane
            print(f"Has right lane: True (lane type: {right_lane.lane_type})")
            return True
    ...
    def draw_control_panel(self):
        ...
        try:
            ...
            if self.sensor_manager and self.vehicle and self.vehicle.is_alive:
                lane_type = self.sensor_manager.get_current_lane_type()
                lane_type_text = self.font.render("Lane Type: {}".format(lane_type), True, self.YELLOW)
