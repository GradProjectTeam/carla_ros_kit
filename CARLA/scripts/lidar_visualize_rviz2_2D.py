...
try:
    ...
    # Add LIDAR sensor with updated configuration for 2D road detection
    lidar_bp = blueprint_library.find('sensor.lidar.ray_cast')
    lidar_bp.set_attribute('channels', '32')          # More channels for better coverage
    lidar_bp.set_attribute('points_per_second', '100000')  # Increased for better resolution
    lidar_bp.set_attribute('rotation_frequency', '50')    # Faster rotation for smoother viz
    lidar_bp.set_attribute('range', '400.0')          # Increased range for better visibility
    lidar_bp.set_attribute('upper_fov', '0')         # Slight upward view
    lidar_bp.set_attribute('lower_fov', '-15')       # Wider FOV for better road detection

    # Print LIDAR configuration
    print("\nLIDAR Configuration (2D Road Detection):")
    ...
    # Spawn LIDAR with adjusted position (centered on vehicle roof)
    lidar_location = carla.Transform(
        carla.Location(x=0.0, z=2.4),  # Centered on roof
        carla.Rotation(pitch=0.0)       # Level positioning
    )
    lidar = world.spawn_actor(lidar_bp, lidar_location, attach_to=vehicle)
    actor_list.append(lidar)
    print("\nLIDAR sensor added at position: x=0.0m, z=2.4m")
    ...
    def lidar_callback(point_cloud):
        ...
        try:
            ...
            # Filter points near ground level (more permissive range)
            road_mask = (-1.0 < points[:, 2]) & (points[:, 2] < 0.5)
            ...
            if len(road_points) > 0:
                try:
                    ...
                    header = struct.pack('!I', len(road_points))
                    points_data = np.ascontiguousarray(road_points, dtype=np.float32).tobytes()
                    client_socket.sendall(header + points_data)
                    
                    print("\rPoints visualized: {}".format(len(road_points)), end='')
