...
try:
    ...
    def lidar_callback(point_cloud):
        ...
        try:
            ...
            # Visualize points in CARLA simulator
            for point in points:
                # Create CARLA location object with explicit float values
                loc = carla.Location(
                    float(point[0]),  # x
                    float(point[1]),  # y
                    float(point[2])   # z
                ...
                # Draw point in simulator (red for road points, blue for others)
                if -0.45 < point[2] < -0.25:  # Road height range
                    world.debug.draw_point(
                        world_loc,
                        size=0.05,  # Point size
                        color=carla.Color(r=255, g=0, b=0),  # Red
                        life_time=0.1  # Duration points remain visible
                    )
                else:
                    world.debug.draw_point(
                        world_loc,
            ...
            print("\rVisualized points: {}".format(len(points)), end='')
